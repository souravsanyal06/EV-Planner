...
def dotie():
   ...
   if flag:
      ...
      x1_p = dotie_x.max()
      y1_p = dotie_y.max()
      cv2.rectangle(dotie_image, (y0_p, x0_p), (y1_p, x1_p), (255, 255, 0), 1)#, cv2.LINE_AA)
      ...
      # Bounding box coordinates in numpy.
      # ground_truth_bbox_numpy = np.array([1202, 123, 1650, 868], dtype=np.float32)

      
      states = rospy.ServiceProxy('/gazebo/get_model_state', GetModelState)
      y_D = states('moving_ring','').pose.position.y
      
      #1.7 is the ring diameter
      ...
      prediction_bbox_numpy = np.array([X0_P, Y_0, X1_P, Y_1], dtype=np.float32)
      ground_truth_bbox_numpy = np.array([X_0, Y_0, X_1, Y_1], dtype=np.float32)

      iou = get_iou(ground_truth_bbox_numpy, prediction_bbox_numpy)
      if iou > 0.6:
          IOU.append(iou)
      print('mIOU: ', sum(IOU)/len(IOU))
      print('peak IOU: ', max(IOU))
